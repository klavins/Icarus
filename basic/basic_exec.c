/*
 * basic_exec.c - BASIC interpreter statement executor
 *
...
int read_line(char *buf, int max) {
    ...
    for (;;) {
        char c = os_read_key();
        if (c == '\n') break;
        if (c == '\b' && pos > 0) { pos--; os_putchar('\b'); continue; }
        if (c && pos < max - 1) { buf[pos++] = c; os_putchar(c); }
    }
    buf[pos] = '\0';
    os_putchar('\n');
    return pos;
...
void exec_tokens(struct token *t) {
    ...
    case TOK_PRINT:
        ...
        while (tok_pos->type != TOK_EOL) {
            if (tok_pos->type == TOK_STRING) {
                os_print(tok_pos->string_val);
                tok_pos++;
            } else if (tok_pos->type == TOK_STRIDENT) {
                os_print(strvar_get(tok_pos->string_val));
                tok_pos++;
            } else {
    ...
    case TOK_GOTO:
        ...
        tok_pos = t;
        {
            ...
            if (idx < 0) { os_print("?UNDEFINED LINE\n"); running = 0; }
    ...
    case TOK_GOSUB:
        ...
        tok_pos = t;
        {
            ...
            int idx = program_find(target);
            if (idx < 0) { os_print("?UNDEFINED LINE\n"); running = 0; }
            else if (gosub_depth >= MAX_GOSUB_DEPTH) { os_print("?GOSUB OVERFLOW\n"); running = 0; }
            else { gosub_stack[gosub_depth++] = next_line_idx; next_line_idx = idx; }
        }
        break;

    case TOK_RETURN:
        if (gosub_depth == 0) { os_print("?RETURN WITHOUT GOSUB\n"); running = 0; }
        ...
        tok_pos = t;
        {
            ...
            if (val >= 1 && val <= n) {
                ...
                if (idx < 0) { os_print("?UNDEFINED LINE\n"); running = 0; }
                    ...
                    if (gosub_depth >= MAX_GOSUB_DEPTH) { os_print("?GOSUB OVERFLOW\n"); running = 0; }
                    else { gosub_stack[gosub_depth++] = next_line_idx; next_line_idx = idx; }
                } else {
                    next_line_idx = idx;
                }
    ...
    case TOK_READ:
        ...
        while (t->type != TOK_EOL) {
            if (data_ptr >= data_count) {
                os_print("?OUT OF DATA\n");
                running = 0;
                return;
            }
            if (t->type == TOK_IDENT && t[1].type == TOK_LPAREN) {
                if (data_store[data_ptr].is_string) { os_print("?TYPE MISMATCH IN READ\n"); running = 0; return; }
                const char *name = t->string_val;
                tok_pos = t + 2;
                int i1 = parse_int();
                int i2 = 0;
    ...
    case TOK_SOUND:
        ...
        tok_pos = t;
        {
            ...
            (void)voice;
            if (volume == 0) os_tone_off();
            else os_tone(atari_pitch_to_hz(pitch));
        }
        break;
